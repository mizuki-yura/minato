...
> **ベータ版です。**
> 大きな仕様変更が入る可能性があり、更新するとゴーストが動かなくなることがあります。
> 配布するゴーストで使う場合は、バージョンを固定し、更新前にバックアップを取ってください。
...
湊は MIT ライセンスで公開しています（`LICENSE` を参照）。
個人・商用を問わず、自由に使用・改変・再配布できます。
配布するときは、著作権表示とライセンス文（`LICENSE`）を同梱してください。
湊を使って作ったゴーストの内容（台詞・画像など）の権利は、作者に帰属します。
